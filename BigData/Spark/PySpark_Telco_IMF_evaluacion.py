...
# MAGIC
# MAGIC %md ##Desarrollo del Caso Practico - Herramientas para Big Data - Spark##
# MAGIC ###Programa de Master en Data Science & Business Analytics- Modalidad Virtual###
# MAGIC **Estudiante Richard Douglas Grijalba** 
# MAGIC
...
# MAGIC
# MAGIC ### Solicitudes o Cuestiones a resolver 
# MAGIC
# MAGIC **A continuacion una breve descripcion de las tareas a resolver en este problema de Data Science**
# MAGIC
# MAGIC * **caso 1:**  Mostrar por pantalla el número total de clientes del mes anterior con más de un contrato con la compañía.
# MAGIC * **caso 2:** Generar un nuevo dataframe de facturas del mes actual que asigne un 7 % de descuento a todos los contratos
# MAGIC de clientes que ya existían en el mes anterior (los contratos de los clientes nuevos seguirán con el mismo
# MAGIC importe). Mantener la columna "importe" y crear una nueva columna "importe_dto" con el nuevo importe,
# MAGIC casteada a dos decimales. Mostrar, además, el resultado ordenado por los campos “id_cliente” ascendente e
# MAGIC importe descendente.
# MAGIC * **caso 3:** Por problemas de saturación en la red debido al incremento en el uso de datos por el confinamiento debido a la
# MAGIC COVID-19, se decide limitar el uso de datos este mes subiendo la tarifa de todas las ofertas que incluyan "datos
# MAGIC ilimitados" en un 15 %. Obtener un DF con las mismas columnas que el DF de facturas del mes actual y, a
# MAGIC mayores, la columna "importe_dto".
# MAGIC * **caso 4:** Crear una nueva variable "grupo_edad" que agrupe a los clientes, tanto del “mes_actual” como del
# MAGIC “mes_anterior” en cuatro rangos según su edad asignando valores del uno al cuatro según el rango al que
# MAGIC pertenezcan (18-25 (1), 26-40 (2), 41-65 (3), >65 (4)).
# MAGIC Obtener una tabla resumen que extraiga para cada para cada uno de los cuatro grupos identificados la media
# MAGIC de consumo de datos, SMS enviados, “minutos_movil”, “minutos_fijo”, con todos los campos casteados a dos
# MAGIC decimales y ordenar el DF por “grupo_edad” ascendente. Extraer conclusiones.
# MAGIC
# MAGIC * **caso 5:** Se quiere realizar un estudio por sexo para analizar si son las mujeres o los hombres quienes consumen más
# MAGIC datos durante el fin de semana y hacen más llamadas desde el móvil. Para ello, se deberá, sin ayuda de un
# MAGIC calendario, extraer el día de la semana al que corresponde cada una de las fechas del mes de agosto para
# MAGIC saber cuáles son fin de semana (se consideran días de fin de semana el viernes, sábado y domingo).
# MAGIC El DF a obtener deberá tener dos registros con las siguientes columnas: sexo, “total_mins_movil_finde”,
# MAGIC “total_datos_moviles_finde”. Extraer conclusiones tras presentar el DF resultante
# MAGIC
# MAGIC * **caso 6:** Obtener un DF que contenga cuatro registros, que serán el cliente de cada grupo edad que más datos móviles
# MAGIC ha consumido durante los 15 primeros días del mes de agosto (día 15 incluido en el cálculo).
# MAGIC El DF deberá contener las columnas: nombre, edad, “grupo_edad”, “datos_moviles_total_15”,
# MAGIC “max_sms_enviados_15” (“max_sms_enviados_15” contiene el máximo de sms enviados en un día por el
# MAGIC cliente con más “datos_moviles” consumidos de cada “grupo_edad” durante esos 15 primeros días del mes).
# MAGIC Extraer conclusiones en cuanto a datos consumidos y SMS enviados por cada “grupo_edad”.
# MAGIC
# MAGIC * **caso 7:** Interesa averiguar los minutos de llamadas de los clientes que son nuevos este mes para realizar un estudio del
# MAGIC impacto que tendrán en caso de producirse un pico de volumen de llamadas en la red.
# MAGIC Obtener un DF que contenga solo a los clientes nuevos de este mes con cuatro columnas:
# MAGIC “nombre_cliente_nuevo”, edad, “importe_total_mes_actual”, “total_minutos”.
# MAGIC Se ordenará el DF resultante por la columna "total_minutos", que contiene el total de minutos de llamadas fijas y
# MAGIC móviles durante este mes para cada cliente, ordenándolos de manera que el primer cliente nuevo que aparezca
# MAGIC sea el que menos minutos de llamadas ha realizado en el mes de agosto.
# MAGIC
# MAGIC * **caso 8:** Obtener un DF que contenga, para los clientes que ya existían en el mes anterior y siguen dados de alta este
# MAGIC mes, tres columnas: nombre, edad, “n_dias_sin_sms”. La última columna se refiere a obtener el número de días
# MAGIC en los que el cliente no ha enviado ningún SMS durante el mes de agosto. Forzar al comando show a que
# MAGIC muestre 30 valores.
# MAGIC Si en todo el mes el cliente "A" no envió SMS tres días, esta columna deberá contener el valor “3”. Si hubiera
# MAGIC algún cliente que hubiese enviado al menos un SMS todos los días del mes, esta columna tendrá valor “0”).
# MAGIC
# MAGIC * **caso 9:** Se desea obtener un coeficiente de ponderación que permita evaluar a cada cliente en función de su consumo
# MAGIC para identificar los clientes más atractivos que forman parte de la compañía.
# MAGIC Este cálculo solo se realizará para los clientes que tengan una sola oferta contratada con la compañía.
# MAGIC Este coeficiente se obtendrá en base a los consumos diarios y, por tanto, solo se tendrán en cuenta los clientes
# MAGIC que existen en el mes de agosto, ya que no existen datos de consumo del mes de julio. Las ponderaciones que
# MAGIC se darán a cada uno de los consumos son las siguientes:
# MAGIC 0,4 --> llamadas desde telefóno móvil.
# MAGIC 0,3 --> “datos_móviles_MB”.
# MAGIC 0,2 --> llamadas desde teléfono fijo.
# MAGIC 0,1 --> SMS enviados.
# MAGIC
# MAGIC * **caso 10:** Se desea averiguar la fecha en la que entre los tres clientes que más datos consumen de cada “grupo_edad”
# MAGIC llegan a un consumo de 20 GB de datos móviles. En caso de que algún “grupo_edad” no llegue, entre los tres
# MAGIC clientes, a 20 GB en todo el mes, se asignará null como valor de esta columna (1 GB = 1024 MB).
# MAGIC Obtener un DF que contiene cuatro registros, uno para cada “grupo_edad” y tres columnas: “grupo_edad”,
# MAGIC “fecha_20_GB”, “datos_moviles_total_grupo_3_clientes”.
# MAGIC “datos_moviles_total_grupo_3_clientes” representa el total de datos consumidos en MB por los tres clientes del
# MAGIC grupo hasta final de mes).
...
# MAGIC %md
# MAGIC ### Informacion General del Problema de Negocio## 
# MAGIC
# MAGIC Una compañía de telecomunicaciones quiere obtener insights de la muestra de 30 clientes obtenida al final de este mes y con información también del mes anterior. Los analistas de datos tendrán que agregar esta información, que se proporciona para responder a 10 cuestiones que ha planteado la directiva.
# MAGIC
# MAGIC **Se inicia conformando, llamando, enunciando y guardando los data frames o data sets necesarios para el desarrollo del caso o problema de negocio que se nos presenta**
# MAGIC
# MAGIC La informacion que tenemos para este caso o problema de negocio corresponde a : 
# MAGIC
# MAGIC **Las tablas de input son las mismas que se utilizaron en el ejercicio guiado de Spark:**
# MAGIC
# MAGIC * **df_clientes**: Muestra información de cada cliente (id, nombre, edad, ciudad, pais).
# MAGIC * **df_facturas_mes_ant**: Muestra la factura del mes anterior junto con el plan que tenían contratado para cada cliente. Un mismo cliente puede tener contratadas dos ofertas con la compañía, en cuyo caso habrá dos filas (de ahora en adelante, registros) con el mismo “id_cliente” y distinto “id_oferta”.
# MAGIC * **df_facturas_mes_actual**: Es una tabla equivalente a la anterior, pero muestra la información actualizada a último día del mes actual (agosto de 2020).
# MAGIC * **df_consumos_diarios**: Muestra para cada cliente, y diariamente, los datos móviles consumidos, el tráfico, SMS enviados y minutos de llamadas a fijos y móviles durante el mes en curso.
# MAGIC * **df_ofertas**: Muestra un catálogo de los distintos paquetes ofrecidos por la empresa, junto con una descripción y sus precios.
# MAGIC
...
# MAGIC %md
# MAGIC **DataFrame Ofertas estandar (truncate=False)**
...
# MAGIC %md **DataFrame facturas_mes_ant**
...
# MAGIC %md **DataFrame factura_mes_actual**
...
# MAGIC %md **DataFrame consumos_diarios** 
# MAGIC
...
# MAGIC %md **Caso 1. Mostrar por pantalla el número TOTAL de clientes que tenían en el mes anterior más de un contrato con la compañía**
...
# MAGIC %md
# MAGIC **Estos son los clientes que tienen mas de un contrato**  Esto no es solicitado, pero lo aplico como un metodo de verificacion, que en total tenemos 8 clientes que presentan mas de un contrato
...
# MAGIC %md
# MAGIC **Vamos a mostrar los mismos resultados pero con solo dos decimales**
...
# MAGIC %md **En este caso vamos a agregar el rango de las edades em el DF de clientes**
# MAGIC
...
# MAGIC %md **Para cada uno de los 4 grupos identificados la MEDIA de consumo de datos, sms enviados, minutos_movil, minutos_fijo, con todos los campos casteados a 2 decimales y ordenar el DF por grupo_edad ascendente. Extraer conclusiones.**
...
# MAGIC %md
# MAGIC ### Conclusiones sobre el resultado del resumen anterior
# MAGIC
# MAGIC rangos según su edad asignando valores del 1 - 4 segun el rango al que pertenezcan (18-25 (1), 26-40 (2), 41-65 (3), >65 (4))

# COMMAND ----------

# MAGIC %md 
# MAGIC * **Media Consumo Datos**  : **El grupo de edad - 1  18 a 25**, presenta la media de consumo de datos mas alta, y la misma disminuye a mayor rango de edad
# MAGIC * **Media Mensajes Enviados SMS** : **El grupo numero 4** mayor a 65. Es que presenta un mayor uso de los mensajes de texto con una media 6.
# MAGIC * **Media de Minutos Movil** : Los grupos de eda 2 y 3 son los que mas consumen los mintiso movil presentando una media de 34.88 los del grupo 2, y una media 32.44 los del grupo 3.
# MAGIC * **Media de Minutos Fijo** : Aquellos que pertenecen a los grupo de edad 4. presentan el mayor consumo de minutos fijo
# MAGIC
# MAGIC Este breve ejercicio permite conocer con una breve exploracion de los datos, el comportamiento de consumo en telefonia por las personas segmentadas por la edad, a lo que se le puede dar seguimiento y la empresa podria generar
# MAGIC promociones segun los ragos de preferencia de la segmentacion de los clientes por edad.
# MAGIC
# MAGIC
# MAGIC
...
# MAGIC %md
# MAGIC *El DF a obtener deberá tener 2 registros con las columnas: sexo, total_mins_movil_finde, total_datos_moviles_finde.*
...
# MAGIC %md ### Conclusiones de los resultados  
# MAGIC
# MAGIC Segun el cuadro resumen anterior se denotan algunas diferencias claras :
# MAGIC
# MAGIC * **Total de minutos moviles**  Se denota un mayor consumo por parte de las mujeres en el total de llamadas o minutos movil, Mujeres un total de 6732 minutos contra los Hombres 3303, praticamente un consumo del doble
# MAGIC
# MAGIC * **Total de Datos Moviles**  Existe un consumo similar entre ambos, siendo en este caso nuevamente las mujeres las que consumen mas los datos los fines de semana
# MAGIC
# MAGIC
# MAGIC Estos resultados nos permite determinar que durante los fines de semana los clientes tendran mayor consumo de datos y llamadas , y las muejeres van a demandar un mayor cantidad de llamadas, la segmentacion y promociones de los clientes se puede hacer tomando en cuenta esta informacion.
# MAGIC
...
# MAGIC ### Conclusines
# MAGIC
# MAGIC El cleinte que nos dice la tabla resumen con mayor conusmo de datos moviles corresponde a **Carlos** de 21 años 
# MAGIC
# MAGIC Los clientes que tienen un mayor consumo, comportamiento en general, que se pueden asignar como VIP's son importantes para cualquier empresa, ademas las empresas deben buscar fidelizacion de los clientes para que auqellos clientes A,B o C, se mantengan o aumenten de categoria
# MAGIC
# MAGIC Esto permite a las empresas garantizar un monto promedio de facturacion de servicios, disminucion de costos en atencion de quejas.
# MAGIC
# MAGIC
...
# MAGIC %md 
# MAGIC **Una vez que tenemos el Df con los clientes que estan presentes en ambos meses, se procede a crear el  proceso para el calculo de los dias sin envio de sms**
# MAGIC
...
# MAGIC %md
# MAGIC **Obtener la suma de todos los días de cada uno de los 4 consumos para cada cliente.**
